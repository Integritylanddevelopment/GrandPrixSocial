...
class MemoryContextRouterAgent:
    ...
    def load_relevant_content(self, matches: list, user_prompt: str = "") -> str:
        ...
        try:
            ...
            if self.logic_config.get("ai_tagging_system", {}).get("enabled", False):
            ...
            for match in matches:
                ...
                if isinstance(match, dict):
                    ...
                    # Legacy string-based match
                    content = self._load_legacy_content(match)
                    if content:
                        content_blocks.append(content[:500] + "...")
                        current_chars += len(content)
            
            result = "\n\n---\n\n".join(content_blocks)
            logging.info(f"Loaded {len(content_blocks)} content blocks ({current_chars} chars)")
            return result
        ...
        try:
            ...
            for match in matches[:10]:  # Limit to top 10 matches
                ...
                if os.path.exists(semantic_file):
                    ...
                    if lines:
                        content_parts.append(f"=== {match} (semantic) ===\n{''.join(lines[:50])}\n")
                        files_processed += 1
                        continue
                ...
                if os.path.exists(working_file):
                    ...
                    if lines:
                        content_parts.append(f"=== {match} (working) ===\n{''.join(lines[:25])}\n")
                        files_processed += 1
            
            self.state["search_statistics"]["files_processed"] += files_processed
            full_content = "\n".join(content_parts)
            
            logging.info(f"Loaded content from {files_processed} files, total size: {len(full_content)} chars")
            return full_content
