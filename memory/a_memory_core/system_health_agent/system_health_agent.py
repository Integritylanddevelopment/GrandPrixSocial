...
class SystemHealthAgent:
    ...
    def check_dependencies(self):
        ...
        if missing_dependencies:
            self.logger.info(f"🔧 Auto-repairing: Installing {len(missing_dependencies)} missing dependencies...")
    ...
    def install_missing_dependencies(self, missing_deps):
        ...
        if successful_installs:
            self.logger.info(f"🎉 Successfully installed: {', '.join(successful_installs)}")
        
        if failed_installs:
            self.logger.warning(f"⚠️  Failed to install: {[dep for dep, _ in failed_installs]}")
        
        return successful_installs, failed_installs
    
    def check_critical_components(self):
        """Check critical system components"""
        self.logger.info("🏗️  Checking critical system components...")
    ...
    def check_agent_integrity(self):
        ...
        self.logger.info("🤖 Checking agent integrity...")
            ...
            for agent_file in self.base_dir.glob(pattern):
                ...
                try:
                    ...
                    if integrity_score >= 66:
                        self.logger.info(f"✅ {agent_name}: Healthy ({integrity_score:.0f}%)")
                    else:
                        self.logger.warning(f"⚠️  {agent_name}: Degraded ({integrity_score:.0f}%)")
                    ...
                    self.logger.error(f"❌ {agent_name}: Corrupted - {e}")
    ...
    def save_health_report(self, report):
        ...
        self.logger.info(f"📊 Health report updated: {latest_file}")
        ...
        while True:
            try:
                ...
                if health in ['POOR', 'CRITICAL']:
                    self.logger.warning(f"⚠️  System health is {health} - repair may be needed")
                else:
                    self.logger.info(f"✅ System health is {health}")
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print(f"📦 Dependencies: {len(report['dependencies']) - dep_issues}/{len(report['dependencies'])} OK")
        print(f"🏗️  Components: {len(report['components']) - comp_issues}/{len(report['components'])} OK")
        print(f"🤖 Agents: {len(report['agents']) - agent_issues}/{len(report['agents'])} OK")
