...
def analyze_memory_system():
    ...
    for memory_type, description in memory_types.items():
        ...
        if type_path.exists():
            print(f"  ✅ {memory_type:<20} - {description}")
        else:
            print(f"  ❌ {memory_type:<20} - {description} (MISSING)")
    ...
    if core_path.exists():
        for agent_dir in core_path.iterdir():
            if agent_dir.is_dir() and not agent_dir.name.startswith('.'):
                ...
                status = "✅ ACTIVE" if agent_files['main_script'] else "⚠️  INACTIVE"
    ...
    for memory_type in ['b_long_term_memory', 'c_short_term_memory', 'd_working_memory', 'e_episodic', 'e_procedural', 'f_semantic', 'g_episodic']:
        ...
        if type_path.exists():
            ...
            print(f"    ✅ {memory_type:<20} - {file_count} items")
        else:
            print(f"    ❌ {memory_type:<20} - Directory missing")
...
CREATE TABLE IF NOT EXISTS f1_specific_data (
    ...
    # Generate indices
    indices = [
        "CREATE INDEX IF NOT EXISTS idx_memory_importance ON memory_core_storage(importance_score DESC);",
        "CREATE INDEX IF NOT EXISTS idx_memory_tags ON memory_core_storage USING gin(tags);",
        "CREATE INDEX IF NOT EXISTS idx_memory_access ON memory_core_storage(last_accessed DESC);",
